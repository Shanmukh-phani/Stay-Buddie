...
// API endpoint to create a new hostel
router.post('/create', async (req, res) => {
  const {
      hostel_name,
      hostel_type,
...
// Backend: Implementing Pagination with Sorting
...
router.get('/rooms', verifyToken, async (req, res) => {
  ...
  // router.get('/rooms', verifyToken, async (req, res) => {
  //   const { hostel_id, page = 1, limit = 10 } = req.query;
  //   const skip = (page - 1) * limit;
  
  //   try {
  //     const rooms = await Room.find({ hostel_id }).skip(skip).limit(limit);
  //     res.json(rooms);
  //   } catch (error) {
...
router.put('/updateRoom/:id', verifyToken, async (req, res) => {
  ...
  router.get('/buddies', verifyToken, async (req, res) => {
    ...
    try {
      // Fetch buddies with the specified hostel_id and where approved is true
  ...
  // Route to fetch all unapproved buddies
...
// Get count of unapproved buddies based on hostel_id
